let themes = {
	a                                                                                                              : {
		forum: {
			color: '#302F2F'
		},
		grey : {
			color: '#313138'
		},
		green: {
			color: '#04700C'
		},
		pink : {
			color: '#A03B3B'
		}
	},
	'.battery-level,#battery-cap'                                                                                  : {
		forum: {
			background: '#302F2F'
		},
		grey : {
			background: '#313138'
		},
		green: {
			background: '#04700C'
		},
		pink : {
			background: '#A03B3B'
		}
	},
	body                                                                                                           : {
		blue    : {
			background: '#2B5177'
		},
		forum   : {
			background: '#CCD0D5'
		},
		grey    : {
			background: '#949494'
		},
		green   : {
			background: '#D0F8AA'
		},
		contrast: {
			background: '#81007f'
		},
		orange  : {
			background: '#FCA335'
		},
		pink    : {
			background: '#FFB5D6'
		},
		purple  : {
			background: '#99509C'
		},
		red     : {
			background: '#CE4040'
		}
	},
	'#header,#footer,#playlists-tabs,.search-in-playlist'                                                          : {
		forum: {
			color: '#302F2F'
		},
		grey : {
			color: '#313138'
		},
		green: {
			color: '#04700C'
		},
		pink : {
			color: '#A03B3B'
		}
	},
	'.nav-ul-li:hover,#header-left:hover,.search-in-playlist:hover,#header-right .nav-ul-a:hover,.menu-ul-li:hover': {
		blue    : {
			background: '#3D84CC'
		},
		forum   : {
			background: '#F1F1F1'
		},
		grey    : {
			background: '#E7E7E7'
		},
		green   : {
			background: '#B7DB9B'
		},
		contrast: {
			background: '#fffd00'
			, color   : '#000'
		},
		orange  : {
			background: '#FFB557'
		},
		pink    : {
			background: '#FDE7F1'
		},
		purple  : {
			background: '#C470C9'
		},
		red     : {
			background: '#F55252'
		}
	},
	'.playlists-tabs-li'                                                                                           : {
		blue    : {
			background: '#355D85'
		},
		forum   : {
			background: '#E1E7EF'
		},
		grey    : {
			background: '#C9C9C9'
		},
		green   : {
			background: '#B7DB9B'
		},
		contrast: {
			background: '#000'
		},
		orange  : {
			background: '#FFB557'
		},
		pink    : {
			background: '#FDC5DE'
		},
		purple  : {
			background: '#BA69BD'
		},
		red     : {
			background: '#BD3636'
		}
	},
	'.playlists-tabs-li.active'                                                                                    : {
		blue                 : {
			background: '#3B78B6'
		},
		'forum,grey,contrast': {
			background: '#fff'
		},
		green                : {
			background: '#EAF1E5'
		},
		contrast             : {
			color: '#000'
		},
		orange               : {
			background: '#FFCC8B'
			, color   : '#FFF'
		},
		pink                 : {
			background: '#FFD8EA'
		},
		purple               : {
			background: '#D297D5'
		},
		red                  : {
			background: '#F18282'
		}
	},
	'.playlist'                                                                                                    : {
		blue    : {
			background: '#44709C'
		},
		forum   : {
			background: '#E4EAF2'
		},
		grey    : {
			background: '#C7C6C6'
		},
		green   : {
			background: '#FFF'
		},
		contrast: {
			background: '#000'
		},
		orange  : {
			background: '#FFF7EC'
		},
		pink    : {
			background: '#FAD4E5'
		},
		purple  : {
			background: '#D8B6DA'
		},
		red     : {
			background: '#FFBEBE'
		}
	},
	'.playlist-item'                                                                                               : {
		forum   : {
			color: '#000'
		},
		grey    : {
			color: '#683D0E'
		},
		green   : {
			color: '#04700C'
		},
		contrast: {
			color       : '#fffd00'
			, background: '#000'
		},
		orange  : {
			color: '#9C4308'
		},
		pink    : {
			color: '#A03B3B'
		}
	},
	'.playlist-item:nth-of-type(odd)'                                                                              : {
		blue  : {
			background: '#355D85'
		},
		forum : {
			background: '#E1E7EF'
		},
		grey  : {
			background: '#C9C9C9'
		},
		green : {
			background: '#E0FCCC'
		},
		orange: {
			background: '#FDD9AA'
		},
		pink  : {
			background: '#FDC5DE'
		},
		purple: {
			background: '#BA69BD'
		},
		red   : {
			background: '#F18282'
		}
	},
	'.playlist-item:nth-of-type(even)'                                                                             : {
		blue  : {
			background: '#376A9C'
		},
		forum : {
			background: '#E7EDF5'
		},
		grey  : {
			background: '#D5D5D5'
		},
		green : {
			background: '#ECFDDF'
		},
		orange: {
			background: '#F8DFC0'
		},
		pink  : {
			background: '#FAD4E5'
		},
		purple: {
			background: '#C477C9'
		},
		red   : {
			background: '#F56E6E'
		}
	},
	'.playlist-item.selected'                                                                                      : {
		blue           : {
			background: '#569BDF'
		},
		forum          : {
			background: '#F9F9F9'
		},
		'grey,contrast': {
			background: '#EBD9B4'
			, color   : '#000'
		},
		'green,orange' : {
			background: '#6DAA41'
			, color   : '#fff'
		},
		contrast       : {
			background: '#fff'
		},
		orange         : {
			background: '#FCAE4C'
		},
		pink           : {
			background: '#FFF2F2'
		},
		purple         : {
			background: '#A759AC'
		},
		red            : {
			background: '#EC3C3C'
		}
	},
	'.window'                                                                                                      : {
		blue    : {
			'border-color': '#174575'
		},
		forum   : {
			'border-color': '#B6B6B6'
		},
		grey    : {
			'border-color': '#ADADAD'
		},
		green   : {
			'border-color': '#7ABD49'
		},
		contrast: {
			'border-color': '#00f'
		},
		orange  : {
			'border-color': '#F59013'
		},
		pink    : {
			'border-color': '#FFA0CB'
		},
		purple  : {
			'border-color': '#88388B'
		},
		red     : {
			'border-color': '#C53232'
		}
	},
	'#preferences-tabs'                                                                                            : {
		blue    : {
			'border-color': '#6EAAE6'
			, background  : '#7BB4EC'
		},
		forum   : {
			'border-color': '#CFCFCF'
			, background  : '#ddd'
		},
		grey    : {
			'border-color': '#B9B9B9'
			, background  : '#C7C7C7'
		},
		green   : {
			'border-color': '#B0E4B0'
			, background  : '#D5F8BB'
		},
		contrast: {
			'border-color': '#fffd00'
			, background  : '#fff'
		},
		orange  : {
			'border-color': '#F8BC6E'
			, background  : '#FDCE92'
		},
		pink    : {
			'border-color': '#FFC0DC'
			, background  : '#FDECF4'
		},
		purple  : {
			'border-color': '#CE86CF'
			, background  : '#E5ADE6'
		},
		red     : {
			'border-color': '#F57A7A'
			, background  : '#FAA3A3'
		}
	},
	'.preferences-item.active'                                                                                     : {
		blue            : {
			background: '#3E86CF'
		},
		'forum,contrast': {
			background: '#E6E6E6'
			, color   : '#000'
		},
		grey            : {
			background: '#E7E7E7'
			, color   : '#313138'
		},
		green           : {
			background: '#B7DB9B'
			, color   : '#04700C'
		},
		contrast        : {
			background: '#fffd00'
		},
		orange          : {
			background: '#FCAB49'
		},
		pink            : {
			background: '#FFCDE3'
			, color   : '#A03B3B'
		},
		purple          : {
			background: '#CB81CF'
		},
		red             : {
			background: '#F55252'
		}
	},
	'#preference-general-help,#preference-language-help,.help-icon'                                                : {
		contrast: {
			color: '#fffd00'
		}
	},
	'#find-window-results'                                                                                         : {
		blue    : {
			background: '#355D85'
		},
		forum   : {
			background: '#E1E7EF'
		},
		grey    : {
			background: '#C9C9C9'
		},
		green   : {
			background: '#EAF1E5'
		},
		contrast: {
			background: '#000'
		},
		orange  : {
			background: '#FFCC8B'
		},
		pink    : {
			background: '#FDC5DE'
		},
		purple  : {
			background: '#BA69BD'
		},
		red     : {
			background: '#FCDCDC'
		}
	},
	'#window-title'                                                                                                : {
		blue    : {
			background: '#174575'
		},
		forum   : {
			background: '#B6B6B6'
		},
		grey    : {
			background: '#ADADAD'
		},
		green   : {
			background: '#7ABD49'
		},
		contrast: {
			background: '#00f'
		},
		orange  : {
			background: '#F59013'
		},
		pink    : {
			background: '#FFA0CB'
		},
		purple  : {
			background: '#88388B'
		},
		red     : {
			background: '#C53232'
		}
	},
	'#window-buttons'                                                                                              : {
		blue    : {
			'border-color': '#6EAAE6'
			, background  : '#ABCEF1'
		},
		forum   : {
			'border-color': '#CFCFCF'
			, background  : '#ECECEC'
		},
		grey    : {
			'border-color': '#B9B9B9'
			, background  : '#E2E2E2'
		},
		green   : {
			'border-color': '#B0E4B0'
			, background  : '#C9F0B6'
		},
		contrast: {
			'border-color': '#fffd00'
			, background  : '#fffd00'
		},
		orange  : {
			'border-color': '#F8BC6E'
			, background  : '#FFD8A6'
		},
		pink    : {
			'border-color': '#FFC0DC'
			, background  : '#FCDDEB'
		},
		purple  : {
			'border-color': '#CE86CF'
			, background  : '#E6BAE7'
		},
		red     : {
			'border-color': '#F57A7A'
			, background  : '#FFB0B0'
		}
	},
	'#volume,#progress'                                                                                            : {
		blue    : {
			background: '#355D85'
		},
		forum   : {
			background: '#E1E7EF'
		},
		grey    : {
			background: '#C9C9C9'
		},
		green   : {
			background: '#EAF1E5'
		},
		contrast: {
			background: '#000'
		},
		orange  : {
			background: '#FFCC8B'
		},
		pink    : {
			background: '#FDC5DE'
		},
		purple  : {
			background: '#BA69BD'
		},
		red     : {
			background: '#FCDCDC'
		}
	},
	'#volume-progress,#playback-progress'                                                                          : {
		blue  : {
			background: '#5A92CA'
		},
		green : {
			background: '#B7DB9B'
		},
		orange: {
			background: '#FFB557'
		},
		pink  : {
			background: '#FFD8EA'
		},
		purple: {
			background: '#D297D5'
		},
		red   : {
			background: '#F18282'
		}
	},
	'.add-item.active'                                                                                             : {
		blue                       : {
			background: '#1E6CB9'
		},
		'forum,green,contrast,grey': {
			background: '#fff',
			color     : '#000'
		},
		grey                       : {
			color: '#313138'
		},
		orange                     : {
			background: '#FFB557'
		},
		pink                       : {
			background: '#FFD8EA'
			, color   : '#A03B3B'
		},
		purple                     : {
			background: '#EDD4EE'
			, color   : '#C769CF'
		},
		red                        : {
			background: '#F18282'
		}
	},
	'.add-item span'                                                                                               : {
		blue    : {
			color: '#1861AA'
		},
		green   : {
			color: '#358D35'
		},
		contrast: {
			color: '#fff'
		},
		orange  : {
			color: '#F08700'
		},
		pink    : {
			color: '#924F6D'
		},
		purple  : {
			color: '#860288'
		},
		red     : {
			color: '#CC1A1A'
		}
	},
	'.add-item.active span'                                                                                        : {
		forum   : {
			color: '#C7C7C7'
		},
		grey    : {
			color: '#9B9B9B'
		},
		green   : {
			color: '#504848'
		},
		contrast: {
			color: '#000'
		},
		orange  : {
			color: '#FAFAFA'
		},
		pink    : {
			color: '#FF94C4'
		},
		purple  : {
			color: '#C769CF'
		},
		red     : {
			color: '#F5DEDE'
		}
	},
	h6                                                                                                             : {
		blue    : {
			color: '#0C4885'
		},
		forum   : {
			color: '#9C6B6B'
		},
		grey    : {
			color: '#FDF8F8'
		},
		green   : {
			color: '#7ABD49'
		},
		contrast: {
			color: '#fffd00'
		},
		orange  : {
			color: '#FCEEDD'
		},
		pink    : {
			color: '#A03B3B'
		},
		purple  : {
			color: '#FBECFC'
		},
		red     : {
			color: '#B31212'
		}
	},
	'#playlist-hints'                                                                                              : {
		forum : {
			color: '#302F2F'
		},
		grey  : {
			color: '#313138'
		},
		green : {
			color: '#04700C'
		},
		orange: {
			color: '#000'
		},
		pink  : {
			color: '#A03B3B'
		}
	},
	'#console-title'                                                                                               : {
		blue    : {
			background: '#ABCEF1'
		},
		forum   : {
			background: '#ECECEC'
		},
		grey    : {
			background: '#E2E2E2'
		},
		green   : {
			background: '#C9F0B6'
		},
		contrast: {
			background: '#fffd00'
		},
		orange  : {
			background: '#FFD8A6'
		},
		pink    : {
			background: '#FCDDEB'
		},
		purple  : {
			background: '#E6BAE7'
		},
		red     : {
			'border-bottom': '1px solid #000'
			, background   : '#FFB0B0'
			, color        : '#000'
		}
	},
	'#add-playlist'                                                                                                : {
		blue    : {
			background      : '#3E86CF'
			, 'border-color': '#2B5177'
		},
		forum   : {
			background      : '#E6E6E6'
			, 'border-color': '#CCD0D5'
		},
		grey    : {
			background      : '#E7E7E7'
			, 'border-color': '#949494'
		},
		green   : {
			background      : '#A0E771'
			, 'border-color': '#D0F8AA'
		},
		contrast: {
			background      : '#fffd00'
			, 'border-color': '#81007f'
			, color         : '#000'
		},
		orange  : {
			background      : '#FDBF73'
			, 'border-color': '#FCA335'
		},
		pink    : {
			background      : '#FFCDE3'
			, 'border-color': '#FFB5D6'
		},
		purple  : {
			background      : '#CB81CF'
			, 'border-color': '#99509C'
		},
		red     : {
			background      : '#F55252'
			, 'border-color': '#CE4040'
		}
	},
	'.menu-ul-li'                                                                                                  : {
		blue    : {
			background: '#2B5177'
		},
		forum   : {
			background: '#CCD0D5'
		},
		grey    : {
			background: '#949494'
		},
		green   : {
			background: '#D0F8AA'
		},
		contrast: {
			background: '#81007f'
		},
		orange  : {
			background: '#FCA335'
		},
		pink    : {
			background: '#FFB5D6'
		},
		purple  : {
			background: '#99509C'
		},
		red     : {
			background: '#CE4040'
		}
	},
	'#console-panel'                                                                                               : {
		blue    : {
			'border-color': '#6EAAE6'
		},
		forum   : {
			'border-color': '#CFCFCF'
		},
		grey    : {
			'border-color': '#B9B9B9'
		},
		green   : {
			'border-color': '#B0E4B0'
		},
		contrast: {
			'border-color': '#fffd00'
		},
		orange  : {
			'border-color': '#F8BC6E'
		},
		pink    : {
			'border-color': '#FFC0DC'
		},
		purple  : {
			'border-color': '#CE86CF'
		},
		red     : {
			'border-color': '#F57A7A'
		}
	},
	'#window-content'                                                                                              : {
		'blue,contrast': {
			background: '#598DC0'
			, color   : '#fff'
		},
		forum          : {
			background: '#DADADA'
		},
		grey           : {
			background: '#CECECE'
		},
		green          : {
			background: '#E7FFD7'
		},
		contrast       : {
			background: '#000'
		},
		orange         : {
			background: '#FCC27B'
		},
		pink           : {
			background: '#FDCDE3'
		},
		purple         : {
			background: '#CD98CF'
		},
		red            : {
			background: '#F18080'
		}
	},
	'.css3-selectbox select'                                                                                       : {
		blue            : {
			background: '#D3E9FF'
		},
		'green,contrast': {
			background: '#fff'
			, color   : '#04700C'
		},
		contrast        : {
			color: '#000'
		},
		orange          : {
			background: '#FFF3E4'
		},
		pink            : {
			background: '#FFF7FB'
		},
		purple          : {
			background: '#F8D9F8'
		},
		red             : {
			background: '#F7D4D4'
		}
	},
	'.css3-selectbox:before'                                                                                       : {
		blue            : {
			'border-color': '#D3E9FF'
			, color       : '#D3E9FF'
		},
		'green,contrast': {
			'border-color': '#fff'
		},
		orange          : {
			'border-color': '#FFF3E4'
		},
		pink            : {
			'border-color': '#FFF7FB'
		},
		purple          : {
			'border-color': '#F8D9F8'
		},
		red             : {
			'border-color': '#F7D4D4'
		}
	},
	'.css3-selectbox:after'                                                                                        : {
		blue    : {
			color           : '#598DC0'
			, 'border-color': '#6B6B83'
		},
		forum   : {
			color           : '#DADADA'
			, 'border-color': '#C7C7C7'
		},
		grey    : {
			color           : '#CECECE'
			, 'border-color': '#AFADAD'
		},
		green   : {
			color           : '#04700C'
			, 'border-color': '#04700C'
		},
		contrast: {
			color: '#000'
		},
		orange  : {
			color           : '#FCC27B'
			, 'border-color': '#FCCA7F'
		},
		pink    : {
			color           : '#FDCDE3'
			, 'border-color': '#FF94C4'
		},
		purple  : {
			color           : '#CD98CF'
			, 'border-color': '#C769CF'
		},
		red     : {
			color           : '#F18080'
			, 'border-color': '#F07676'
		}
	},
	'.css3-selectbox.top-bar select'                                                                               : {
		blue            : {
			background: '#3E86CF'
		},
		'forum,contrast': {
			background: '#E6E6E6'
			, color   : '#000'
		},
		green           : {
			background: '#fff'
			, color   : '#04700C'
		},
		grey            : {
			background: '#E7E7E7'
			, color   : '#313138'
		},
		contrast        : {
			background: '#fffd00'
		},
		orange          : {
			background: '#FCEEDD'
			, color   : '#9C4308'
		},
		pink            : {
			background: '#FFCDE3'
			, color   : '#A03B3B'
		},
		purple          : {
			background: '#CB81CF'
		},
		red             : {
			background: '#F55252'
		}
	},
	'.css3-selectbox.top-bar:before'                                                                               : {
		blue            : {
			'border-color': '#3E86CF'
		},
		'forum,contrast': {
			'border-color': '#E6E6E6'
		},
		green           : {
			'border-color': '#fff'
		},
		grey            : {
			'border-color': '#E7E7E7'
		},
		contrast        : {
			'border-color': '#fffd00'
		},
		orange          : {
			'border-color': '#FCEEDD'
		},
		pink            : {
			'border-color': '#FFCDE3'
		},
		purple          : {
			'border-color': '#CB81CF'
		},
		red             : {
			'border-color': '#F55252'
		}
	},
	'.css3-selectbox.top-bar:after'                                                                                : {
		'forum,contrast': {
			color           : '#000'
			, 'border-color': '#C7C7C7'
		},
		green           : {
			color: '#04700C'
		},
		grey            : {
			color           : '#313138'
			, 'border-color': '#AFADAD'
		},
		contrast        : {
			'border-color': '#000'
		},
		orange          : {
			color           : '#9C4308'
			, 'border-color': '#FCCA7F'
		},
		pink            : {
			color           : '#A03B3B'
			, 'border-color': '#FF94C4'
		},
		purple          : {
			'border-color': '#F5E1F7'
		},
		red             : {
			'border-color': '#FAD2D2'
		}
	},
	'.window-title-span'                                                                                           : {
		forum: {
			color: '#000'
		},
		grey : {
			color: '#313138'
		},
		pink : {
			color: '#A03B3B'
		}
	},
	'#connected-table tr td:first-child'                                                                           : {
		blue    : {
			color: '#1861AA'
		},
		green   : {
			color: '#358D35'
		},
		contrast: {
			color: '#fff'
		},
		orange  : {
			color: '#F08700'
		},
		pink    : {
			color: '#924F6D'
		},
		purple  : {
			color: '#860288'
		},
		red     : {
			color: '#CC1A1A'
		}
	},
	button                                                                                                         : {
		blue                  : {
			background: '#D3E9FF'
		},
		'forum,green,contrast': {
			background: '#fff'
		},
		grey                  : {
			background: '#FAFAFA'
		},
		orange                : {
			background: '#FFF3E4'
		},
		pink                  : {
			background: '#FFF7FB'
		},
		purple                : {
			background: '#F8D9F8'
		},
		red                   : {
			background: '#F7D4D4'
		}
	},
	'#keyboard-shortcuts button:not(#shortcut-add)'                                                                : {
		blue  : {
			color: '#3D84CC'
		},
		grey  : {
			color: '#747474'
		},
		green : {
			color: '#588337'
		},
		orange: {
			color: '#FFB557'
		},
		pink  : {
			color: '#A03B3B'
		},
		purple: {
			color: '#C470C9'
		},
		red   : {
			color: '#F55252'
		}
	},
	'#translation-contributors'                                                                                    : {
		blue    : {
			background: '#6EA8E2'
		},
		forum   : {
			background: '#ECECEC'
		},
		grey    : {
			background: '#E2E2E2'
		},
		green   : {
			background: '#C9F0B6'
		},
		contrast: {
			background: '#fffd00'
			, color   : '#000'
		},
		orange  : {
			background: '#FFD8A6'
		},
		pink    : {
			background: '#FCDDEB'
		},
		purple  : {
			background: '#E6BAE7'
		},
		red     : {
			background: '#FFB0B0'
		}
	},
	'.tooltip:hover:after'                                                                                         : {
		blue        : {
			background: 'rgba(12,50,107,.9)'
		},
		'forum,grey': {
			background: 'rgba(255,255,255,.9)'
			, color   : '#000'
		},
		grey        : {
			color: '#313138'
		},
		green       : {
			background: 'rgba(195,246,158,.93)'
			, color   : '#04700C'
		},
		contrast    : {
			background: '#81007f'
		},
		orange      : {
			background: 'rgba(231,148,41,.9)'
		},
		pink        : {
			background: 'rgba(255,155,195,.92)'
		},
		purple      : {
			background: 'rgba(139,53,143,.9)'
		},
		red         : {
			background: 'rgba(162,21,21,.9)'
		}
	},
	'.tooltip:hover:before'                                                                                        : {
		blue        : {
			'border-color': 'rgba(12,50,107,.9) transparent'
		},
		'forum,grey': {
			'border-color': 'rgba(255,255,255,.9) transparent'
		},
		green       : {
			'border-color': 'rgba(195,246,158,.93) transparent'
		},
		contrast    : {
			'border-color': '#81007f transparent'
		},
		orange      : {
			'border-color': 'rgba(231,148,41,.9) transparent'
		},
		pink        : {
			'border-color': 'rgba(255,155,195,.92) transparent'
		},
		purple      : {
			'border-color': 'rgba(139,53,143,.9) transparent'
		},
		red         : {
			'border-color': 'rgba(162,21,21,.9) transparent'
		}
	},
	'#init-search'                                                                                                 : {
		grey : {
			background: '#E7E7E7'
		},
		green: {
			background: '#E9EBE8'
		},
		pink : {
			background: '#FFF0F8'
		}
	},
	'#window-close'                                                                                                : {
		green: {
			color: '#fff'
		}
	},
	'.playlists-tabs-li.active a,.menu-ul-li:hover,.nav-ul-li:hover a'                                             : {
		contrast: {
			color: '#000'
		}
	},
	'#drop-zone.hover'                                                                                             : {
		blue    : {
			'border-color': '#006400'
		},
		contrast: {
			'border-color': '#fffd00'
		}
	},
	'#footer-counter'                                                                                              : {
		blue    : {
			'border-color': '#6B6B83'
		},
		green   : {
			'border-color': '#04700C'
		},
		contrast: {
			'border-color': '#00f'
		},
		orange  : {
			'border-color': '#FCCA7F'
		},
		pink    : {
			'border-color': '#FF94C4'
		},
		purple  : {
			'border-color': '#C769CF'
		},
		red     : {
			'border-color': '#F07676'
		}
	},
	'.selected.is-in-queue .item-queue'                                                                            : {
		'forum,grey,contrast,pink': {
			'color': '#fff'
		}
	},
	'.favicon'                                                                                                     : {
		contrast: {
			background: '#fff'
		}
	}
};